...
namespace SimpleCMSWeb.Controllers
{
    public class PatientAccountController : Controller
    {
        ...
        public async Task<IActionResult> Register(RegistrationFormVM model)
        {
            ...
            {
                ModelState.AddModelError("Municipalityid", "Please select a municipality.");
                model.Municipalities = await _context.Municipalities.Where(m => (bool)m.IsActive).ToListAsync();
                ViewData["Step"] = 2;
                return View(model);
            }
            ...
            var patient = new Patient
            {
                ...
                Suffixname = model.Suffixname ?? null,
                Birthdate = model.Birthdate,
                Gender = model.Gender,
                Civilstatus = model.Civilstatus,
                Mobileno = model.Mobileno,
                Homeaddress = model.Homeaddress,
                Municipalityid = model.Municipalityid ?? 0,
                Philhealthno = model.Philhealthno ?? null,
        ...
        public async Task<IActionResult> ForgotPassword(ForgotPasswordVM model)
        {
            ...
            if (!string.IsNullOrEmpty(model.NewPassword) && !string.IsNullOrEmpty(model.ConfirmPassword))
            {
                ...
                matchedUser.Password = model.NewPassword;
                await _context.SaveChangesAsync();
